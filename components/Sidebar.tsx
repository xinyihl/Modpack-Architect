...
import { Plus, Search, Cpu, Filter, Languages, Settings } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ onDefineRecipe, onEditRecipe, onOpenSettings, activeRecipeId }) => {
  ...
  return (
    <div className="flex flex-col border-r border-zinc-800 bg-zinc-900 w-80 shrink-0 h-full">
      <div className="h-16 flex items-center justify-between px-4 border-b border-zinc-800 shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={onOpenSettings} className="p-2 text-zinc-500 hover:text-white transition-colors">
            <Settings size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="px-4 pb-4 shrink-0 space-y-2 border-b border-zinc-800/50 mb-2">
      ...
      <div className="flex-1 overflow-y-auto px-2">
        ...
        <div className="space-y-1">
          {filtered.length > 0 ? filtered.map(recipe => (
            <div key={recipe.id} onClick={() => onEditRecipe(recipe.id)} className={`group p-2 rounded cursor-pointer border transition-colors ${activeRecipeId === recipe.id ? 'bg-blue-900/20 border-blue-800' : 'hover:bg-zinc-800 border-transparent hover:border-zinc-700'}`}>
