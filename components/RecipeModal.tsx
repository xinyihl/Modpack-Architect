        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-tighter ${step === 'select-machine' ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-500'}`}>
                {t('modal.stepMachine')}
              </span>
              <ChevronRight size={12} className="text-zinc-700" />
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-tighter ${step === 'config' ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-500'}`}>
                {t('modal.stepConfig')}
              </span>
            </div>
          </div>
          <button onClick={onClose} className="p-2 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors"><X size={20} /></button>
        </div>
        <div className="flex-1 p-6">
          {step === 'select-machine' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="text-center space-y-2"><h3 className="text-zinc-300 font-medium italic text-sm">{t('modal.selectMachinePrompt')}</h3></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {machines.map((machine) => (
                  <button key={machine.id} onClick={() => handleSelectMachine(machine.id)} className={`flex items-start text-left gap-4 p-4 rounded-xl border transition-all group ${selectedMachineId === machine.id ? 'bg-blue-600/10 border-blue-500 shadow-lg shadow-blue-500/10' : 'border-zinc-800 bg-zinc-800/20 hover:bg-zinc-800/50 hover:border-zinc-700'}`}>
                    <div className={`p-3 rounded-lg transition-colors ${selectedMachineId === machine.id ? 'bg-blue-600 text-white' : 'bg-zinc-800 group-hover:bg-blue-600/20 group-hover:text-blue-400'}`}><Cpu size={24} /></div>
                    <div>
                      <h4 className={`font-bold transition-colors ${selectedMachineId === machine.id ? 'text-white' : 'text-zinc-200 group-hover:text-white'}`}>{machine.name}</h4>
